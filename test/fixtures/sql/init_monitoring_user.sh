...
# this script, see test/compose/topo-standalone.yml) with the :'pw' psql
# variable it requires — docker-entrypoint-initdb.d runs .sql files with a
...
# provide.
set -e
psql -v ON_ERROR_STOP=1 -v pw="${PGLENS_MONITORING_PASSWORD:-pglens-monitoring-test}" \
