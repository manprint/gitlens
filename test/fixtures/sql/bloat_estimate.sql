WITH widths AS (
  SELECT schemaname, tablename,
         COALESCE(sum(avg_width * (1 - null_frac)), 0)::float8 AS tuple_width,
         count(*) FILTER (WHERE null_frac > 0)::int AS nullable_columns
  FROM pg_stats GROUP BY schemaname, tablename
), relations AS (
  SELECT n.nspname AS schemaname, c.relname, c.reltuples, c.relpages,
         c.oid, pg_total_relation_size(c.oid)::float8 AS real_bytes,
         COALESCE(w.tuple_width, 0)::float8 AS tuple_width,
...
SELECT schemaname, relname, '' AS indexrelname, 'table' AS object_kind,
       real_bytes, GREATEST(reltuples * (tuple_width + 23), 0) AS expected_bytes,
