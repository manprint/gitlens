...
_Last updated: 2026-09-02 — sub-phase 17.7 OPEN; README authentication examples in progress._
...
| **ID** | 17.7 |
| **Status** | `OPEN` |
| **Intent** | Rewrite every README API example so it authenticates explicitly and is runnable by an operator. |
| **Next action:** | Complete the README API authentication rewrite, validate the examples against a live server, and pass the `agent-1:opus` review gate. |
...
Sub-phase 17.4 is closed: the server image measured 9.54 MiB before the embedded
frontend and 10.16 MiB after it, the runtime image contains no Node executable,
both release images built for `linux/amd64` and `linux/arm64` in 268s, the
release workflow consumes the complete CI gate, and both deployment Compose
files validate with an empty UI password by default. The next unit is
sub-phase 17.5.

Sub-phase 17.5 is closed: Vite keeps ECharts and React Flow in lazy vendor
...
Sub-phase 17.7 is open: the README API examples are being rewritten to show
session-cookie authentication (with the bootstrap-token equivalent) before
any protected request. The required review gate is `agent-1:opus`.
...
Sub-phase 17.4 verified the pre- and post-frontend server image sizes, the
Node-free runtime image, both release image architectures, the reusable CI gate,
the two deployment Compose files, and the documented Q-C/Q-D limits. No
production source change was required.

Sub-phase 17.5 added `web/scripts/bundle-budget.ts` and its parser test,
split ECharts and React Flow into lazy Vite vendor chunks, added the
`web-budget` Make target to the local and CI quality chains, and added the
Playwright-specific typed-lint override that retains the fixed-wait ban. It
also corrected pre-existing Prettier drift in `web/src/test/fixtures/getLocks.ts`
so the repository web gate is reproducibly green.

---

## §6 — In-flight work

`claimed — sub-phase 17.7 README authentication rewrite in progress; preserve the operator-facing scope and validate every protected curl example.`
...
| 2026-09-01 | 17.4 | Baseline/current `Dockerfile.server` builds and runtime checks | PASS | Baseline server image: 9.54 MiB; embedded-frontend image: 10.16 MiB; `--version` returned `dev`; `/usr/bin/node` was absent from the runtime image. |
| 2026-09-01 | 17.4 | `make build-images-multiarch` | FAIL → PASS | Initial local attempt failed in 10s because the default builder lacked `arm64` emulation; after registering QEMU, agent and server `linux/amd64,linux/arm64` builds passed in 268s. |
| 2026-09-01 | 17.4 | Release workflow, Compose, and limits review | PASS | `release-alpha.yml` gates publish on reusable `ci.yml`, both Compose files parse with an empty UI password by default, and `server.example.env`/`docs/LIMITS.md` document UI authentication, Q-C, and Q-D. |
| 2026-09-01 | 17.4 closure | `go test -tags=e2e -timeout=8m -count=1 ./test/e2e/... -run '^TestFull_DeployExampleStack$'` | PASS | SYS-DEPLOY-001 passed in 42s wall-clock (Go test 41.751s); residual `pglens-*` and `pglens-deploy-*` containers: 0/0. |
| 2026-09-01 | 17.5 | `pnpm --dir web exec vitest run src/lib/bundle-budget.test.ts` | PASS | UI-PERF-001 and UI-PERF-002 parser tests passed. |
| 2026-09-01 | 17.5 | Initial `make web-lint web-typecheck` | FAIL — corrected | The E2E override removed typed-boundary diagnostics, then the gate exposed two intentional browser `fetch` calls and pre-existing Prettier drift in `getLocks.ts`; both were corrected without weakening the fixed-wait rule. |
...
| 17 | [phase_18.md](phase_18.md) | Packaging, UI acceptance suite, documentation | `agent-2:sonnet` / `agent-3:haiku` | 17.2, 17.7, 17.8 | IN PROGRESS — 6/8 sub-phases closed; 17.7 OPEN |
...
| 17.7 | Rewrite the README's API examples for authentication | `agent-3:haiku` | IN PROGRESS |
