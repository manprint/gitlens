...
- **Build:** `make build` · **Fmt:** `make fmt-check` · **Lint:** `make lint` · **Vet under every tag:** `make vet-tags`
- **Supply chain:** `make tidy-check` · `make vuln` — *added by row 91*
- **Race stress:** `make stress` — *added by row 91*
...
- **Toolchain:** Go 1.27.1 · Docker 29.7.2 · Docker Compose v5.5.0
...
| 91 | review | R001 (full-project correctness and staging review, user-requested; the agent-side half is plan 001 row 168) | agent-1:opus | **Server-side defects, each closed with a regression test.** **(a) gzip decompression bomb on `/api/v1/push`.** `MaxBytesReader` bounded the 32 MiB wire body, but nothing bounded the body *after* gzip, and the ratio on the repetitive JSON an envelope is made of is easily three orders of magnitude — one authenticated (or merely corrupted) request could expand to tens of gigabytes and OOM-kill the server. Added `maxDecompressedBytes` (256 MiB, 8x the wire limit) enforced with `io.ReadAll(io.LimitReader(gz, max+1))`, answering 413 — the status the agent's `pushOne` already treats as "drop, do not retry" — and counting `pglens_ingest_rejected_total{reason="decompressed_too_large"}`. A `var` rather than a `const` purely so the bomb test lowers it and asserts the boundary in ~1s instead of inflating a real 256 MiB. **(b) No panic recovery on the HTTP router.** A panic in any handler killed the process; `middleware.Recoverer` is now the first middleware. **(c) No HTTP timeouts on either server.** `http.Server` was used bare on the server and the agent health endpoint: a Slowloris client held a connection indefinitely. Both now set `ReadHeaderTimeout`/`ReadTimeout`/`WriteTimeout`/`IdleTimeout`/`MaxHeaderBytes`. **(d) Unbounded session store:** swept only lazily, so a login loop grew it without limit. Added `sessionReapThreshold` (1024) and a hard `sessionHardCap` (4096) that evicts the sessions closest to expiry first. **(e) Unbounded topology engine:** `Pipeline.maybeEvict` aged out delta state but never topology state, so instances, per-instance `edgeSeenAt` and per-cluster failover/split-brain state accumulated forever. Added `topology.Engine.Evict(before)`/`Len()` and wired `p.topo.Evict(cutoff)` into `maybeEvict`. **(f) Lifecycle:** `Staleness`, the alert engine and the advisor engine could each `close(doneCh)` twice or wait on a channel `Start` never created; all three now carry explicit `started`/`stopped` state, proven by lifecycle tests under `internal/leaktest`. `alert.Engine.Tick` also read and wrote `stateLoaded` outside the mutex. **(g) Connection-pool floor:** the server pins 3 connections for leader election; a DSN with a small `pool_max_conns` could starve every other query. `main.go` now parses the config, raises `MaxConns` to a usable floor (skipped when the DSN sets `pool_max_conns` explicitly), sets `HealthCheckPeriod`, and replaces a `log.Fatalf` inside the listener goroutine with a `listenErr` channel so shutdown is orderly. **(h) Toolchain:** `go.mod` now pins `toolchain go1.26.6`, closing 15 standard-library advisories reachable from the ingest endpoint, the agent's HTTP client and the notification channels; `make vuln` is what proves the pin still holds. **Harness.** `test/harness/metrics.go` (new, **no build tag**) parses the server's Prometheus exposition and asserts a `MetricBudget`; `AssertInvariants` now calls it, which makes **I-8 (cardinality within budget) and "no unexpected check errors" real for the first time** — row 89's `docs/LIMITS.md` and phase 6's invariant list had both carried them as permanently unreachable gaps because `pglens_series_total`/`pglens_check_error_total` did not exist when they were written. They exist now, so the gap note is retired rather than restated. **CI and docs.** `ci.yml` gained a `security` job (`tidy-check` + `govulncheck`), `vet-tags` in `lint` (the `integration`/`e2e` suites are never compiled by `go build ./...`), a `race-stress` job (10 shuffled `-race` rounds, off pull requests), an `images` job that builds both Dockerfiles and runs `--version` inside them, coverage artefacts on `if: always()`, `persist-credentials: false` on every checkout, a cached pinned `golangci-lint`, and a `ci-ok` aggregate so branch protection covers jobs added later. `e2e.yml` reclaims ~25 GB of preinstalled runner toolchains before pulling images and captures `docker ps`/`df -h` on failure. `release-alpha.yml` publishes with SBOM and provenance and records digests in the job summary. `docs/LIMITS.md` gained limit 13 (every in-memory ceiling and what it trades), `TESTING.md` §11/§12 now describe the workflows that exist rather than a `pr.yml` that does not, and README's L5 was corrected from "performance and profiling" to the Playwright suite it actually is | internal/server/{ingest.go,http.go,session.go,staleness.go,pipeline.go}, internal/topology/engine.go, internal/alert/{eval.go,engine.go}, cmd/pglens-server/main.go, go.mod, Dockerfile.{server,agent}, test/harness/{metrics.go (new),invariants.go}, .github/workflows/{ci.yml,e2e.yml,release-alpha.yml}, Makefile, scripts/coverage_gate.sh, README.md, TESTING.md, CONTRIBUTING.md, docs/LIMITS.md | `make fmt-check lint vet-tags build test coverage-gate tidy-check vuln` clean; `make stress` clean; 26/26 packages ok under `-race -shuffle=on`; coverage 76.8% global, every floor OK; web typecheck/lint/prettier clean, 629 web tests pass, UI coverage gate pass | `b7e92da` |

| 92 | review | R001-F02 (follow-on to row 91: making the two invariants row 91 introduced actually decidable) | agent-1:opus | Row 91 wired up "no unexpected check errors" and it failed its first scenario, SYS-UI-001, with eleven checks each reporting one error and every browser assertion green. The rule was wrong, not the product: a scenario that fails over a primary, partitions a link or restarts a container *makes* every check against that instance error once, and that is correct behaviour. Three iterations, each one exposed by the run after the last. **(a) Added `pglens_check_ok_total`** (server-side counter, incremented in both `Pipeline` result loops and exposed by `metrics_handler.go`), because the failure this invariant exists to catch — `stat_statements` erroring on every scrape for months with `wire.Result.Error` read by nothing — is "errored and never once succeeded", and without a success counter that shape is not expressible. **(b) The invariant must survive a server the scenario took away.** Three full-suite failures were `I-8: scrape /metrics: connection refused`, from two separate causes: scenarios that deliberately stop the server (now skipped via a `docker compose ps --status running` check) and a real harness defect — Compose assigns a *new* ephemeral host port on every `start`, and the port-reconcile path ran only in binary agent mode, so container-mode API clients were left pointing at a dead port after any server restart. Both modes now reconcile, and the metric scrape tries every replica. **(c) One error is not evidence.** `io` runs on a 30s interval and needs PG16+; in a disturbed scenario it gets a single scrape, and when that one failed it was indistinguishable by counter from a check that has never worked — while the same scenario had passed on the previous run. `minErrorsToJudgeACheck = 2`: a single unexplained error is logged, not failed. SYS-REPL-004's zero-lag poll was also widened from 20s to 30s, matching every sibling poll in the file, because the pipeline latency it was racing is ~15s (10s check interval + 5s push + server write); the assertion itself is unchanged — the lag must still reach exactly 0 | internal/server/{staleness.go,pipeline.go,metrics_handler.go}, test/harness/{metrics.go,metrics_test.go,invariants.go,harness.go}, test/scenario/replication.go | `go test -race ./test/harness/` green including the four new invariant unit tests; full E2E suite green in both agent modes | `952a7f6`, `e9a4a97`, `036ff9d`, `ecc4147` |

| 93 | review | R001-F03 (follow-on to row 92: what the first clean local full-suite run found that CI never could) | agent-1:opus | **Every PostgreSQL healthcheck was probing the wrong transport.** `pg_isready` with no `-h` asks the Unix socket, and the postgres entrypoint runs initdb and every `/docker-entrypoint-initdb.d` script against a temporary server started with `-c listen_addresses=''` — on the socket and nowhere else. For the width of that window the probe answers "accepting connections", compose marks the service healthy, `depends_on: service_healthy` releases everything downstream, and `pglens-server` exits 1 on `migrate: acquire migration connection: ... dial tcp 172.20.0.3:5432: connect: connection refused` before a TCP listener has ever existed. Five UI scenarios failed this way in a local full-suite run while the same commit was green on all four CI E2E jobs: the race needs an initdb slow enough to outlast `start_period`, which a hosted runner never produced and a loaded workstation did. Fixed with `-h 127.0.0.1` in all seven test-stack healthchecks **and in `deploy/docker-compose.yml`**, where the same race would meet a real operator on the one boot that runs initdb. **The honest probe then exposed the budget.** With the probe no longer able to pass early, `5s start_period + 5x2s` — about fifteen seconds — was simply not enough for a first boot, and the failure moved from a downstream crash to `dependency failed to start: timescaledb is unhealthy`, which is the same race reported in the right place. `start_period` raised to 90s for the databases and 60s for the server; it is not a delay, since a container is marked healthy on the first probe that succeeds and failures inside the window do not count against retries. **One finding withdrawn.** SYS-REPL-005 looked broken locally — the role reversal took 1m40s against a 30s poll — and a pool-recovery policy was written for the agent on the theory that pgxpool's one-minute default `HealthCheckPeriod` was holding a dead socket. Re-measuring disproved it: the change made no difference, and the real cause was a debug stack of mine left running beside the suite for two hours. With it gone the same poll settles in **2s**. The policy was reverted and no test margin was widened — a margin raised to accommodate a measurement artefact would have hidden the next real regression | test/compose/{base.yml,topo-primary-standby.yml,topo-standalone.yml,topo-cascading.yml}, deploy/docker-compose.yml, TESTING.md (regola anti-flake 6) | Full E2E locale verde end to end sotto go1.27.1 (4435s, 0 fallimenti); E2E CI 4/4 verde su `6e2e7e2` in entrambi gli agent mode; CI 14/14 verde | `69b9c71`, `6e2e7e2` |
...
| `internal/check/table_stats.go` | 9.4 | dead-tuple ratio metric and selector truncation metadata |
| `internal/check/index_stats.go` | 9.4 | selector truncation metadata for index cardinality |
| `internal/check/bloat_estimate.go` | 9.4 | estimate method label and selector truncation metadata |
| `internal/check/bloat_estimate.sql` | 9.4 | zero-safe bloat ratio calculation |
| `internal/check/bloat_estimate_test.go` | 9.4 | zero-safe bloat SQL regression assertion |
| `internal/store/migrations/0011_relation_metrics.sql` | 9.4 | persisted dead-tuple ratio and vacuum age columns |
| `internal/store/write.go` | 9.4 | relation metric persistence and stale-row pruning |
| `internal/store/write_test.go` | 9.4 | relation writer argument coverage |
| `internal/server/pipeline.go` | 9.4 | relation metric mapping, labels and stale-row pruning |
| `internal/server/pipeline_test.go` | 9.4 | bloat metric mapping regression coverage |
| `internal/server/api_relations.go` | 9.4 | relation order, bloat method and truncation metadata API |
| `internal/advisor/snapshot.go` | 9.4 | typed relation snapshot loading and index definition hashes |
| `test/compose/base.yml` | 9.4 | advisor interval and relation scrape timing |
...
| `test/fixtures/agent-standalone.yaml` | 9.7 | faster settings and archiver intervals |
| `test/harness/invariants.go` | 9.7 | valid PostgreSQL setting sentinels and metric-name diagnostics |
...
| phase9.4-static | `make fmt-check && make lint && make build && go test ./... && go test -tags=e2e ./test/e2e -run '^$' && go vet -tags=e2e ./test/e2e/...` | PASS — formatting, lint, build, complete Go suite, e2e compile and vet green | 2026-08-30 |
| phase9.4-images | `make build-images` | PASS — agent and server dev images rebuilt with the maintenance implementation | 2026-08-30 |
...
| phase9.7-static | `go vet -tags=e2e ./test/e2e/...` | PASS — e2e vet green | 2026-08-30 |
| phase9.7-images | `make build-images` | PASS — agent and server dev images rebuilt before L3 | 2026-08-30 |
| phase9.7-compile | `go test -tags=e2e ./test/e2e -run '^$'` | PASS — e2e package compiles | 2026-08-30 |
| phase9.7-focused | `go test -tags=e2e -timeout=8m ./test/e2e -run '^TestFull_Archiving$' -count=1 -v` | PASS — SYS-ARCH-001 in 57.09s | 2026-08-30 |
...
| phase10.4-limit-backups | `INT-ARCH-001..003`, `SYS-ARCH-001` and archive metric review | PASS — archiver behavior is covered; no `pg_basebackup` restoreability proof exists, so the limit is explicitly stated | 2026-08-30 |
| phase10.4-limit-bloat | `INT-BLOAT-001..004`, `SYS-BLOAT-001`, `INT-CMD-009` | PASS — estimate, truncation, ownership and optional exact `pgstattuple` paths are covered; full scans remain on demand | 2026-08-30 |
| phase10.4-limit-plans | `INT-CMD-006`, `SYS-CMD-001` and explain executor review | PASS — normalized `pg_stat_statements` input, request-only capture and unexplainable placeholders are represented; parameter invention is absent by design | 2026-08-30 |
| phase10.4-limit-analyze | `INT-CMD-007`, `SYS-CMD-001..002` and the three command gates | PASS — rollback contract and `allow_explain_analyze`/explicit-analyze/tier checks are covered; external side effects are documented as not rollback-safe | 2026-08-30 |
| phase10.4-limit-cardinality | `INT-BLOAT-003`, `SYS-IDX-002`, relation API tests | PASS — rank budget, `truncated` and `relations_not_reported` are asserted; omitted relations are not backfilled | 2026-08-30 |
| phase10.4-limit-host | `INT-HOST-001..005` and host endpoint contract tests | PASS — local host collection and remote `available:false` behavior are covered; PostgreSQL does not supply inferred host memory | 2026-08-30 |
| phase10.4-limit-advisor | `INT-ADV-001..008`, `SYS-ADV-001..004` and rule-catalogue tests | PASS — rule thresholds and reasons are exercised as advice, not correctness verdicts | 2026-08-30 |
...
| D-047 | 9.7 | Use the assigned target pool after PostgreSQL restart | Post-restart queries, workload and repair use `psql` through `e.Exec` inside the PostgreSQL container; the pool is reset but not used for post-restart checks | Compose restart can reassign the published host port; in-container execution makes control independent of a stale host DSN |
| D-048 | 9.7 | Assert I-2 across generic metrics | I-2 excludes only `pg_setting_bytes` and `pg_setting_seconds` from negative values and names the metric in failures | PostgreSQL `temp_file_limit=-1` is a valid gauge sentinel; counters/rates and synthetic `bad_metric` remain covered |
...
| D-038 | 9.4 | Plan names `/api/v1/relations?instance_id=...&order=dead_ratio` | Used the repository's established instance-scoped `/api/v1/instances/{id}/tables`, `/indexes` and `/bloat` routes, adding the planned order/method filters and truncation metadata there | Preserved the existing L3 API contract while exposing every phase-9.4 assertion |
...
| D-060 | 0.2 / 10.5 | The declared Files list did not enumerate every path changed by close commit `06f50ea` | Reconciled the list with the commit's seven changed paths and cross-referenced the capability work to phase 10.3 without rewriting D-059 | V001-F2 fixed in plan 003 |
...
| SYS-ARCH-001 | system | 9 | `DONE` | phase 9 § 9.7 — failing and repaired archiving through the API; three consecutive runs; backup/restore proof explicitly out of scope |
