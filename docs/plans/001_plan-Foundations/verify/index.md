...
| V003 | [verify_003_2026-08-28.md](verify_003_2026-08-28.md) | 2026-08-28 | `FAIL` at audit time; corrected by V003-C01 | 0 | 2 | 1 | `agent-1:opus` |
| V004 | [verify_004_2026-08-28.md](verify_004_2026-08-28.md) | 2026-08-28 | `PASS` | 0 | 0 | 0 | `agent-1:opus` |
...
| V001-F09 | `MINOR` | divergent | Per-check `interval`/`top_n` config is parsed but not consumed | `FIXED` | V003-C01 / V004 | `config.go` getters, scheduler override, `configureChecks`, and config/scheduler tests |
...
| V001-F12 | `MINOR` | divergent | Small disk buffers can permanently latch `buffer_full` | `FIXED` | V003-C01 / V004 | Configured segment sizing, retry after `BufferFull`, and buffer tests |
...
| V001-F14 | `MINOR` | untested | Standby fixture does not attach pre-created physical slot | `FIXED` | V003-C01 / V004 | `standby_init.sh` passes `-S standby1`; replication integration coverage remains green |
...
| V002-F08 | `MINOR` | scope-creep | Test-only DB schema in `setup_test.go` duplicates (does not derive from) the real migrations in `internal/store/migrations/*.sql` | `FIXED` | V003-C01 / V004 | Tagged setup uses TimescaleDB, creates the extension, and calls `store.Migrate` |
| V002-F09 | `MINOR` | rule-violation | `README.md` claims `/readyz` checks migration/pool health, but the handler is static 200 | `FIXED` | V003-C01 / V004 | `API.Ready` checks pool connectivity and applied migrations; HTTP tests cover 200/503 |
...
| V002-F12 | `MINOR` | scope-creep | Repeated test-schema drift caused another integration failure | `FIXED` | V003-C01 / V004 | Server and store integration fixtures use shipped migrations |
...
No implementation blocker or non-blocking finding is currently open. V003's
findings and all six carried-over findings were corrected by V003-C01 and
verified by V004. The repository's `make lint` wrapper now resolves the
installed GOPATH binary and exits cleanly.
